    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="relative">
            <h1 className="text-2xl font-semibold" style={{ color: v("text-primary") }}>Уведомления</h1>
            {unreadCount > 0 && (
              <span className="absolute -top-2 -right-16 flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-bold transition-all duration-300 animate-fade-in" style={{
      ...
      {loading ? (
        ...
        >
          ...
          <p className="mt-3 text-sm" style={{ color: v("text-muted") }}>Нет уведомлений</p>
        </div>
      ) : (
        <div
        ...
        >
          <div className="space-y-1.5">
            {notifs.map((n, i) => {
              const iconInfo = n.source_type === "tax_event"
                ? { icon: AlertTriangle, color: "#ef4444", bg: isDark ? "rgba(239,68,68,0.12)" : "rgba(239,68,68,0.08)" }
                : n.source_type === "calendar_event"
                  ? { icon: Calendar, color: "#6366f1", bg: isDark ? "rgba(99,102,241,0.12)" : "rgba(99,102,241,0.08)" }
                  : { icon: FileText, color: "#10b981", bg: isDark ? "rgba(16,185,129,0.12)" : "rgba(16,185,129,0.08)" };
              const Icon = iconInfo.icon;
              return (
              <div
                key={n.id}
                className={`animate-fade-in stagger-${(i % 6) + 1} group flex items-start justify-between rounded-xl border p-3.5 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg ${
              ...
              >
                <div className="flex items-start gap-3 min-w-0 flex-1 cursor-pointer">
                  <div className="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-lg" style={{ background: iconInfo.bg }}>
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium flex items-center gap-2" style={{ color: v("text-primary") }}>
                      {!n.is_read && (
                        <span className="h-2 w-2 shrink-0 rounded-full" style={{ background: "rgba(99,102,241,0.8)" }} />
                      )}
                      {n.title}
                    </p>
                    {n.body && (
                      <p className="text-xs mt-0.5 line-clamp-2" style={{ color: v("text-muted") }}>{n.body}</p>
                    )}
                    <p className="text-[10px] mt-1" style={{ color: v("text-secondary") }}>
                      {new Date(n.created_at).toLocaleString("ru-RU")}
                      {n.source_type && ` • ${n.source_type === "tax_event" ? "Налог" : n.source_type === "calendar_event" ? "Событие" : n.source_type}`}
                    </p>
                  </div>
                </div>
                <button
